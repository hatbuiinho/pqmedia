<script lang="ts">
	...
	import { PLATFORM_COUNT } from '$lib/constants/platforms';
	import {
		canShareFiles,
		copyToClipboard,
		fetchImagesAsFiles,
	...
	const allPlatformsPublished = $derived(post.publications.length >= PLATFORM_COUNT);
		...
		>
			...
			{#if allPlatformsPublished}
				<p class="mb-3 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">
					...
					Bài này đã được đánh dấu đăng đủ {PLATFORM_COUNT} nền tảng. Vẫn tiếp tục?
				</p>
			{/if}
